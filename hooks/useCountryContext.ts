...
const parseCountry = (value?: string | null): CountryCode | null => {
    ...
    return normalized === 'KZ' ? normalized : null;
};
...
const getCountryFromBrowserUrl = (): CountryCode | null => {
    ...
    const pathCountry = parseCountry(window.location.pathname.match(/^\/(kz)(?=\/|$)/i)?.[1]);
    return pathCountry;
};
...
export function useCountryContext() {
    ...
    const country = useMemo<CountryCode>(() => {
        ...
        const pathCountry = parseCountry(pathname.match(/^\/(kz)(?=\/|$)/i)?.[1]);
        if (pathCountry) {
            return pathCountry;
        }
