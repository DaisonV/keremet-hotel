...
export function middleware(request: NextRequest) {
    ...
    const pathMatch = pathname.match(/^\/(kz)(?=\/|$)/i);
    ...
    // Проект настроен только под Казахстан.
    let country = 'KZ';
    if (pathCountry === 'KZ') {
        country = pathCountry;
    } else if (queryCountry === 'KZ') {
        country = queryCountry;
    } else if (isApiPath && cookieCountry === 'KZ') {
    ...
    if (pathMatch) {
        ...
        rewrittenUrl.pathname = pathname.replace(/^\/(kz)(?=\/|$)/i, '') || '/';
